...
4. **imageSource 只能从上述"可用的配图方式"中选取，严禁使用未列出的方式（使用未列出方式图片生成会失败）**
...
"""
    ...
    # 热门选题生成
    HOT_TOPIC_PROMPT = """你是一位资深内容营销策划师。请根据 {year} 年 {month} 月的时事热点、科技趋势和社会话题，生成 6 个适合写成自媒体爆款文章的选题标题。

要求：
1. 必须紧扣 {year} 年 {month} 月当下真实发生的事件或热门话题（如科技产品发布、政策法规变化、行业重大动态、社会文化现象等）
2. 覆盖科技、商业、职场、生活等不同方向，每个领域最多 2 条
3. 标题要具体、有吸引力，能激发读者点击欲望。严格限制：每条标题 15-30 字（含标点），不得超过 30 字，超长标题会严重影响展示效果
4. 每个 emoji 必须与选题主题精准匹配（如 🤖 科技、💰 商业、💼 职场、🌟 生活、🔥 热点、📱 互联网、🏥 健康、📚 教育）
5. viralScore：爆款潜力指数 1-10（综合考虑话题热度、受众广度、传播性）
6. difficulty：写作难度 1-5（1=门槛低素材多，3=需调研整理，5=需深度专业知识）
7. platforms：推荐发布平台，从以下选择 1-2 个（公众号、知乎、今日头条、小红书、36氪、虎嗅）

请直接返回 JSON 数组，不要有其他内容。注意：topicText 不得超过 30 字。
[{{"emoji": "🤖", "topicText": "选题标题", "viralScore": 8, "difficulty": 3, "platforms": "公众号,知乎"}}, ...]"""
