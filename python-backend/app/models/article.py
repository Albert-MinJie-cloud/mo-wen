...
class Article(Base):
    ...
    task_id = Column(
        "taskId", String(64), nullable=False, unique=True, comment="任务ID（UUID）"
    )
    ...
    full_content = Column(
        "fullContent", Text, nullable=True, comment="完整图文（Markdown格式，含图片）"
    )
    ...
    completed_time = Column(
        "completedTime", DateTime, nullable=True, comment="完成时间"
    )
    update_time = Column(
        "updateTime",
        DateTime,
